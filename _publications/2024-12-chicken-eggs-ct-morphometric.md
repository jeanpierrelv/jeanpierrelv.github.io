---
title: "Alternative Non-Destructive Approach for Estimating Morphometric Measurements of Chicken Eggs from Tomographic Images with Computer Vision"
collection: publications
category: journals
permalink: /publication/2024-12-chicken-eggs-ct-morphometric
excerpt: 'Non-invasive CT image analysis with deep learning for chicken egg morphometric measurements.'
date: 2024-12-01
venue: 'Foods Journal'
paperurl: 'https://doi.org/10.3390/foods13244039'
citation: 'Jean Pierre B. López Vargas, Katariny Lima de Abreu, Davi Duarte de Paula, Denis Salvadeo, Lilian Francisco Arantes de Souza, Carlos Bôa-Viagem Rabello. (2024). &quot;Alternative Non-Destructive Approach for Estimating Morphometric Measurements of Chicken Eggs from Tomographic Images with Computer Vision.&quot; <i>Foods Journal</i>.'
---

The egg has natural barriers that prevent microbiological contamination and promote food safety. The use of non-destructive methods to obtain morphometric measurements of chicken eggs has the potential to replace traditional invasive techniques, offering greater efficiency and accuracy.

This paper demonstrates that estimates derived from non-invasive approaches, such as 3D computed tomography (CT) image analysis, can be comparable to conventional destructive methods. Two widely recognized deep learning architectures, U-Net 3D and Fully Convolutional Networks (FCN) 3D, were modeled to segment and analyze 3D CT images of chicken eggs. The models achieved an accuracy of up to 98.69%, demonstrating their effectiveness in egg quality assessment.

[Code on GitHub](https://github.com/jeanpierrelv/CT_Segmentation_Eggs)
